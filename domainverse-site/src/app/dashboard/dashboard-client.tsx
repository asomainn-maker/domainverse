...
export default function DashboardClient({ email, initialProjects }: { email: string; initialProjects: Project[] }) {
  ...
  async function deploy() {
    ...
    try {
      const created = await fetch("/api/projects/create", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: title.trim() }) });
      ...
      setTitle(""); setFiles([]); if (zipInput.current) zipInput.current.value = ""; if (folderInput.current) folderInput.current.value = "";
